...
let int_of_string s =
  try int_of_string s with Failure _ -> failwith (Printf.sprintf "%s is not an int" s)

let int_of_atom a =
  match a with
    Atom s -> int_of_string s
  | _ -> failwith "Un Cons a été donné à la place d'un Atom"

let rec eval_add arg =
    match arg with
      |Atom "nil" -> Atom "0"
      |Atom s1 -> Atom s1
      |Cons(car, cdr) -> Atom (string_of_int ((int_of_atom car) + (int_of_atom (eval_add cdr))))

let eval_sub arg1 arg2 =
	Atom (string_of_int ((int_of_atom arg1) - (int_of_atom arg2)))

let rec eval_mul arg =
	match arg with
	|	Atom "nil" -> Atom "1"
	|	Atom s -> Atom s
	|	Cons(car, cdr) -> Atom (string_of_int ((int_of_atom car) * (int_of_atom (eval_mul cdr))))
